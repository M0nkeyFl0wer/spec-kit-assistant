...
- **FR4.1**: Secure SSH connection to seshat.noosworx.com:8888
...
async function connectToSeshat() {
  // Get credentials from system keyring
  const password = await keytar.getPassword('seshat', 'm0nkey-fl0wer');
...
export SESHAT_HOST=seshat.noosworx.com
export SESHAT_PORT=8888
export SESHAT_USER=m0nkey-fl0wer
...
**Keyring Setup:**
```bash
# Store Seshat password securely
node -e "const keytar = require('keytar'); keytar.setPassword('seshat', 'm0nkey-fl0wer', 'YOUR_PASSWORD');"
```
