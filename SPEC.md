...
async function connectToSeshat() {
  ...
  const password = await keytar.getPassword('seshat', 'REMOTE_USER');
...
export SESHAT_HOST=REMOTE_HOST
export SESHAT_PORT=8888
export SESHAT_USER=REMOTE_USER
...
node -e "const keytar = require('keytar'); keytar.setPassword('seshat', 'REMOTE_USER', 'YOUR_PASSWORD');"
```
