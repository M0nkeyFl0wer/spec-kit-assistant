...
export class SpecFirstInterceptor {
  constructor() {
    ...
    this.enhancedSwarm = new EnhancedSwarmOrchestrator();
  ...
  isValidSpecContent(content) {
    ...
    return specIndicators.some(indicator =>
  ...
  async validateGitHubSpecKitCompliance(specPath) {
    ...
    try {
      ...
      const foundImplementationDetails = implementationKeywords.filter(keyword =>
  ...
  analyzeSpecComplexity(content) {
    ...
    const requirementLines = content.split('\\n').filter(line =>
      line.includes('MUST') || line.includes('SHALL') || line.includes('FR-')
    );
