...
export class SwarmOrchestrator extends EventEmitter {
  ...
  findSuitableAgent(task, preferredAgentType) {
    const availableAgents = Array.from(this.agents.values()).filter((agent) => agent.status === 'ready' || agent.status === 'idle');
  ...
  handleAgentDisconnection(agentId) {
    ...
    const assignedTasks = Array.from(this.tasks.values()).filter((task) => task.assignedAgent === agentId && task.status !== 'completed');
  ...
  requiresOversight(task) {
    ...
    return this.oversightConfig.approvalRequired.includes('*')
           || this.oversightConfig.approvalRequired.some((type) => task.type?.includes(type) || task.description?.toLowerCase().includes(type));
  }
