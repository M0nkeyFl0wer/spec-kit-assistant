...
create_wrapper() {
    ...
    local wrapper_script="/usr/local/bin/spec-kit"

    # Check if we can write to /usr/local/bin
    if [ -w "/usr/local/bin" ] || [ "$(id -u)" = "0" ]; then
        cat > "$wrapper_script" << EOF
#!/bin/bash
# Spec Kit Assistant wrapper script
# Generated by install.sh

export SPEC_KIT_HOME="$INSTALL_DIR"
cd "\$SPEC_KIT_HOME"
node src/index.js "\$@"
EOF
        ...
        mkdir -p "$user_bin"
        wrapper_script="$user_bin/spec-kit"
...
EOF
        chmod +x "$wrapper_script"
        log_success "✅ Wrapper script created: $wrapper_script"
        log_info "Add $user_bin to your PATH to use 'spec-kit' command globally"
    fi
}
...
main() {
    ...
    log_info "🚀 Usage:"
    log_info "   spec-kit              # Start Spec Kit Assistant"
    log_info "   spec-kit come-here    # Start focused session"
    log_info "   spec-kit woof-woof    # Initialize workflow"
    log_info "   spec-kit good-boy     # Positive reinforcement"
    log_info ""
